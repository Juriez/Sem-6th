...
class ConsistentHashChain:
    def __init__(self):
        self.servers = []  # List to hold servers on the hash ring
        self.keys = {}     # Dictionary to map keys to their servers
    ...
    def add_server(self, server_id, position):
        ...
        print(f"Adding server: {server_id} at position {position}")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if choice == "1":
            ...
            position = int(input("Enter position to add server: "))
            hash_chain.add_server(server_id, position)
